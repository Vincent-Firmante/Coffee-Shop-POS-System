...
class AppModel:
    def __init__(self):
        ...
        # credentials are now persisted in the database users table
        self.credentials = {}
        self.user_role = None
    ...
    def remove_item_from_order(self, item_id):
        ...
        Returns:
            bool: True if item was removed, False if item not in order
        """
        if item_id in self.current_order:
    ...
    def restore_archived_eod_summaries(self):
        # Returns number of EOD summaries now present after restore (or 0/None on failure)
        try:
