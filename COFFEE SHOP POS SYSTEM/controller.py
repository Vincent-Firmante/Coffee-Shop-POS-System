...
class AppController:
    ...
    def init_main_window(self):
        ...
        # connect POS menu filter if view provides it
        try:
        ...
        # Connect delete receipt signal if available
        try:
    ...
    def refresh_all_data(self):
        ...
        if self.model.user_role == 'Manager':
            ...
            try:
                ...
                # fallback to previous in-memory credential keys
                self.main_window.update_password_combo([])
        # populate POS category filters for all users
        try:
            self.main_window.update_pos_filters(self.model.get_menu_categories())
        except Exception:
            pass
        # Try to populate transaction history if view supports it
        try:
    ...
    def handle_tab_change(self, tab_name):
        if "End of Day" in tab_name:
            ...
            # load latest receipts when the user opens the Transaction History tab
    ...
    def handle_process_payment(self):
        ...
        if success:
            ...
            self.refresh_all_data()
            # ensure transaction history is updated immediately after a successful payment
            try:
                self.refresh_transaction_history()
            except Exception:
                pass
